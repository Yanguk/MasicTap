...
final class PathResolverTests: XCTestCase {
    ...
    private let fakeExe = "/project/magic-tap/src/client/.build/arm64-apple-macosx/debug/MagicTapClient"
    private let expectedCandidate = "/project/magic-tap/src/backend/zig-out/bin/zig_my_mouse"
    ...
    func test_resolves_release_build_path() {
        let releaseExe = "/project/magic-tap/src/client/.build/arm64-apple-macosx/release/MagicTapClient"
        let expected = "/project/magic-tap/src/backend/zig-out/bin/zig_my_mouse"
        let result = resolveBackendPath(
    ...
    func test_falls_back_when_candidate_not_found() {
        ...
        XCTAssertEqual(result, "../../src/backend/zig-out/bin/zig_my_mouse")
    }

    func test_path_segments_count() {
        // 경로 분해: exe → debug → arch → .build → client → src → project-root
        let exe = "/a/b/src/client/.build/arch/debug/Binary"
        let result = resolveBackendPath(
            executablePath: exe,
            fileExists: { _ in true }
        )
        XCTAssertEqual(result, "/a/b/src/backend/zig-out/bin/zig_my_mouse")
    }

    func test_actual_binary_exists_in_repo() throws {
        // 실제 레포에서 바이너리가 존재하는지 통합 검증
        let actualExe = "/Users/yangukheo/Desktop/1_private/magic-tap/src/client/.build/arm64-apple-macosx/debug/MagicTapClient"
