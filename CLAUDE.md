...
1. **Zig backend** (`src/main.zig`): Listens to multitouch events via the private `MultitouchSupport` framework, detects taps, and fires `CGEvent` mouse clicks.
2. **Swift menu bar client** (`mac-client/`): A SwiftUI `MenuBarExtra` app that launches and manages the Zig backend process.
...
```bash
zig build              # Build → zig-out/bin/zig_my_mouse
zig build run          # Build and run directly
zig build test         # Run tests
```

Requires Zig ≥ 0.15.2 and `local_frameworks/MultitouchSupport.framework` in the repo root (private Apple framework, not committed).

### Swift Client

```bash
cd mac-client
swift run              # Build and launch menu bar app
```

Targets macOS 14+. The client expects the backend binary at `../zig-out/bin/zig_my_mouse` by default (relative to `mac-client/`).

## Architecture

The backend (`src/main.zig`) enumerates MT devices via `MTDeviceCreateList()`, skips built-in trackpads via `MTDeviceIsBuiltIn()`, and only attaches to Magic Mouse devices (PIDs `0x030D` and `0x0269`). It registers `touchCallback` as a `MTContactCallbackFunction` per device.
